...
{
    internal class DatabasePreparedCommandOperation<T> : IOrientDBOperation<CommandResult<T>> where T : OrientDBEntity
    {
        ...
        {
            return _payloadFactory.CreatePayload(_query, _fetchPlan, _metaData, _parameters).CreatePayloadRequest(sessionId, token);
        }
